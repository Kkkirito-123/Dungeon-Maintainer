 ...
 * Hash、worktree、固定检查与试玩均由自有模块强制执行。Runtime 不使用高层 Harness、
...
function tokenTotal(task: TaskRecord): number {
  return task.usage.input + task.usage.output + task.usage.cacheRead + task.usage.cacheWrite;
}

function limitReached(config: RuntimeConfig, task: TaskRecord): boolean {
  return task.usage.turns >= config.maxTurns
    || task.usage.toolCalls >= config.maxToolCalls
    || tokenTotal(task) >= config.maxTokens;
}
...
export async function runAgent(
  ...
  const context: ToolContext = { task, store };
  ...
  const agent = new Agent({
    ...
    beforeToolCall: async ({ assistantMessage, toolCall, args }) => {
      ...
      if (task.usage.toolCalls > config.maxToolCalls || tokenTotal(task) >= config.maxTokens) {
        control.limitStop = true;
        return { block: true, reason: "任务资源限额已达到", terminate: true };
      }
      if (toolCall.name === "patch") {
        if (task.mode !== "fix") return { block: true, reason: "diagnose 任务禁止修改", terminate: true };
    ...
    shouldStopAfterTurn: () => {
      const stop = task.state === "needs_approval" || control.finishStatus !== null || control.limitStop
        || task.usage.turns >= config.maxTurns || tokenTotal(task) >= config.maxTokens;
  ...
  } else if (control.limitStop && task.state !== "blocked" && task.state !== "needs_approval") {
    task.conclusion = "达到模型回合、工具调用或 token 上限。";
