...
export function buildDungeonMaintainerPrompt(): string {
  return [
    ...
    "3. 定位源码默认先用一次 inspect bundle，让架构路由在同一调用中搜索并返回最相关窗口与 baseHash；已知故障楼层时传 floorId。楼层路由依次查当前层、上下相邻层、共享父级 partition，仍不足才回退 area/仓库；只有 bundle 上下文不足时再补 search/read/read_many/diff。同一有效 Hash 的语义重复会返回短回执，不要重复执行已经给出相同结果的动作。",
    ...
    "源码调查优先服从本轮游戏区域路由卡；每层模块只能消费 shared partition 提供的服务，不把相邻层当作可复用依赖，也不要跨楼层复制共享规则。相邻层只用于排查传送、边界和上下层联动。搜索回执为 complete=true 时不要在相同 Hash 下重复搜索。只有三级楼层范围零命中时才按回执回退 area 或全仓。",
    "",
