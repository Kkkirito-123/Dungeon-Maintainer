...
export function buildDungeonMaintainerPrompt(): string {
  return [
    ...
    "3. 定位源码默认先用一次 inspect bundle，让架构路由在同一调用中搜索并返回最相关窗口与 baseHash。优先采用路由卡的 feature；只有功能上下文不足时才扩展到 floor/area/仓库。已知稳定功能或楼层时可传 featureId/floorId；只有 bundle 上下文不足时再补 search/read/read_many/diff。同一有效 Hash 的语义重复会返回短回执，不要重复执行已经给出相同结果的动作。",
    ...
    "源码调查优先服从本轮游戏功能/区域路由卡；每层模块只能消费父级 shared/service provider，不把相邻层当作可复用依赖，也不要跨楼层复制共享规则。相邻层只用于排查传送、边界和上下层联动。搜索回执为 complete=true 时不要在相同 Hash 下重复搜索。",
    "",
