...
export function buildDungeonMaintainerPrompt(): string {
  return [
    ...
    "模型可见工具固定且仅有 inspect、edit、check、finish、workspace、look、act、query；不加载任何 Pi 原生工具或 Bash。工具面保持稳定以复用 Prompt Cache，但写入权限由执行层单独控制。",
    "",
    "硬安全边界：",
    ...
    "7. 完成修改后若无需本轮生成可应用验证结果就直接自然结束，不要调用 finish(status=result)；维护器会像 Pi 一样立即结束本轮，不自动验证或创建隐藏模型回合，代码改动可由用户稍后显式 /verify。只有用户明确要求立即验证时才调用 result；完整固定检查和构建在用户 /apply 前按最终 Hash 运行一次。",
    "",
