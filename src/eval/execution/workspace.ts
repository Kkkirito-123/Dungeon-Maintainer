/**
 * 固定 EvalWorkspace 的安全创建。
 *
 * 本模块只负责从版本库内的 `eval-datasets/<dataset>/` 读取固定
 * manifest、基线文件和补丁，并在调用方指定的全新目录中建立可重复的 Git
 * 评测仓库。它不运行 Eval、不选择任务、不修改 fixture 源数据，也不向
...
interface EvalScenarioManifest {
  schemaVersion: 2;
  id: string;
  base: string;
...
interface EvalSharedBaseManifest {
  schemaVersion: 1;
  id: string;
  sourceCommit: string;
...
export interface EvalWorkspaceOptions {
  /** Scenario ID，必须是不含路径分隔符的单一目录名。 */
  readonly scenarioId: string;
  /** 全新评测仓库的目标目录；其父目录必须已存在。 */
  readonly destination: string;
  /** Dataset 根目录；省略时为当前工作目录下的 `eval-datasets/eval-v1`。 */
  readonly datasetRoot?: string;
  /** `clean` 仅供零模型预检验证 after Oracle；正式 EvalRun 始终省略并使用 broken。 */
  readonly variant?: "broken" | "clean";
}

/** 已建立且完成 manifest 一致性校验的 EvalFixture 仓库事实。 */
...
function parseManifest(value: unknown, requestedId: string): EvalScenarioManifest {
  ...
  if (
    ...
    || normalizeScenarioId(manifest.base) !== manifest.base
...
async function readManifest(path: string, requestedId: string): Promise<EvalScenarioManifest> {
  let value: unknown;
  try {
...
async function readSharedBaseManifest(
  path: string,
  requestedId: string,
): Promise<EvalSharedBaseManifest> {
  let value: unknown;
  try {
    value = JSON.parse(await readFile(path, "utf8")) as unknown;
  } catch (error) {
...
interface EvalWorkspaceDefinition {
  readonly manifest: EvalScenarioManifest;
...
async function readWorkspaceDefinition(
  datasetRoot: string,
  id: string,
): Promise<EvalWorkspaceDefinition> {
  const rootInformation = await lstat(datasetRoot);
  if (rootInformation.isSymbolicLink() || !rootInformation.isDirectory()) {
    throw new Error("EvalDataset 根必须是真实目录");
  }
  const realDatasetRoot = await realpath(datasetRoot);
  const scenarioRoot = await realpath(join(realDatasetRoot, "scenarios"));
  assertDirectChild(realDatasetRoot, scenarioRoot, "Scenario 根目录");
  const requestedFixtureDirectory = resolve(scenarioRoot, id);
  ...
  assertDirectChild(scenarioRoot, fixtureDirectory, "Eval Scenario");
  ...
  const requestedBaseDirectory = resolve(realDatasetRoot, "base");
  ...
  assertDirectChild(realDatasetRoot, baseDirectory, "EvalDataset 基线");
...
/**
 * 将单个内置 EvalFixture 物化为可直接运行评测的本地 Git 仓库。
 *
...
export async function createEvalWorkspace(
  ...
  const id = normalizeScenarioId(options.scenarioId);
  const datasetRoot = resolve(
    options.datasetRoot ?? join(process.cwd(), "eval-datasets", "eval-v1"),
  );
  ...
  const definition = await readWorkspaceDefinition(datasetRoot, id);
  const {
    manifest,
    patchPath,
    repositoryPath,
  ...
  try {
    ...
    return { root: destination, baseCommit, dirtyPaths, sourceFingerprint };
  } catch (error) {
    if (created) {
      try {
