...
describe("1.0 任务状态、证据与审批", () => {
  ...
  it("只读取完整的当前任务格式，不迁移旧版本或缺失字段", async () => {
    const repository = await createTemporaryGitRepository({ "README.md": "test\n" });
    try {
      const store = new TaskStore(join(repository.temporaryRoot, "data"));
      const task = await store.create({
        id: "invalid-task-record",
        objective: "验证任务格式门禁",
        repoRoot: repository.repoRoot,
        baseHead: repository.baseHead,
        worktreeRoot: join(repository.temporaryRoot, "worktree"),
        piSessionDir: join(repository.temporaryRoot, "data", "tasks", "invalid-task-record", "pi"),
      });
      const taskPath = join(store.taskDir(task.id), "task.json");
      const current = JSON.parse(await readFile(taskPath, "utf8")) as Record<string, unknown>;
      const unsupported = { ...current, schemaVersion: 3 };
      await writeFile(taskPath, JSON.stringify(unsupported, null, 2) + "\n", "utf8");
      ...
      await writeFile(taskPath, JSON.stringify(missingField, null, 2) + "\n", "utf8");
      await assert.rejects(store.read(task.id), /任务记录格式、ID 或状态非法/u);

      const oldExtendedFormat = { ...current, conclusion: "旧格式结论" };
      await writeFile(taskPath, JSON.stringify(oldExtendedFormat, null, 2) + "\n", "utf8");
      await assert.rejects(store.read(task.id), /任务记录格式、ID 或状态非法/u);

      const incompleteNestedRecord = structuredClone(current);
      delete (incompleteNestedRecord.writeScope as Record<string, unknown>).closedAt;
      await writeFile(
        taskPath,
        JSON.stringify(incompleteNestedRecord, null, 2) + "\n",
        "utf8",
      );
      await assert.rejects(store.read(task.id), /任务记录格式、ID 或状态非法/u);
    } finally {
...
describe("低敏日志与可重放语义 Trace", () => {
  ...
  it("Trace 保持单调序号、固定容量且复现只保存高层动作", async () => {
    ...
    try {
      ...
      await writeFile(
        reproductionPath,
        currentRecord.replace('"schemaVersion": 2', '"schemaVersion": 3'),
        "utf8",
      );
      await assert.rejects(
      ...
      const incomplete = JSON.parse(currentRecord) as Record<string, unknown>;
      delete incomplete.createdAt;
      await writeFile(reproductionPath, JSON.stringify(incomplete, null, 2) + "\n", "utf8");
      await assert.rejects(
        readActiveReproduction(store, evidence, task),
        /复现记录格式、版本或 ID 非法/u,
      );
      const oldExtendedFormat = {
        ...(JSON.parse(currentRecord) as Record<string, unknown>),
        legacyCheckpoint: true,
      };
      await writeFile(
        reproductionPath,
        JSON.stringify(oldExtendedFormat, null, 2) + "\n",
        "utf8",
      );
      await assert.rejects(
        readActiveReproduction(store, evidence, task),
