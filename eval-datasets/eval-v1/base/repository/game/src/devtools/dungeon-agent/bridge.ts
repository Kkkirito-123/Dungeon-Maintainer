...
/**
 * 将当前导航目标压缩为不含 ID、坐标或地图的隐藏判卷语义。
 *
 * @param snapshot 当前隔离游戏快照。
 * @returns 固定四值分类；只供隐藏 Judge 和聚焦测试使用。
 */
export function dungeonAgentGuidanceTargetKind(
  snapshot: GameSnapshot,
): DungeonAgentJudge["guidanceTargetKind"] {
  const objectiveId = snapshot.navigationGuidance.objectiveRoomId;
  if (!objectiveId) return "none";
  if (!objectiveId.startsWith("area-boss:")) return "room";
  const targetMonsterId = Number(objectiveId.slice("area-boss:".length));
  const targetMonster = snapshot.monsters.find((monster) => monster.id === targetMonsterId);
  if (!targetMonster) return "room";
  return targetMonster.hp > 0 ? "live-monster" : "defeated-monster";
}
...
function judgeSnapshot(snapshot: GameSnapshot): DungeonAgentJudge {
  ...
  return {
    ...
    guidanceTargetKind: dungeonAgentGuidanceTargetKind(snapshot),
  };
}
...
export function installDungeonAgentBridge(
  ...
  const bridge: DungeonPlaytestBridge = {
    ...
    async act(revision, actionId, rawMaxSteps) {
      ...
      const action = visible.actions.find((entry) => entry.id === actionId);
      if (!action || action.tool !== "act") {
        trace.record("act", `action=${actionId} result=action-not-available`);
        return result(false, "action-not-available");
      }

      const movementTarget = actionId === "objective" || actionId === "frontier"
        ? actionId
        : null;
      if (
        movementTarget
        && isDungeonAgentVisible(options.root, "#inspection-overlay")
      ) {
      ...
      if (movementTarget) {
        ...
        while (movedSteps < maxSteps) {
          const navigation = planDungeonAgentNavigation(snapshot, movementTarget);
