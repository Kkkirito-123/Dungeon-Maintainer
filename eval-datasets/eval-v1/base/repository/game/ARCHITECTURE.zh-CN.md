...
机器可读路由权威是 `../.maintainer/architecture-map.json`。schema v4 在稳定 layer/area/partition 与
八个跨层 `floorScopes` 之上增加跨层 `features`、有限运行契约和稳定边界签名；它是路由数据，不是
代码或文件索引。普通文件变化无需更新地图。

正常 Inspect 先按 feature 路由，并把楼层作为可选上下文；area/仓库只作失败开放回退。只接受完整的
当前 schema v4。
