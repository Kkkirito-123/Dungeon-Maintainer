...
import {
  dungeonAgentGuidanceTargetKind,
...
describe("Dungeon Agent 玩家投影", () => {
  it("隐藏 Judge 只把导航目标分类为房间、存活怪物或已死亡怪物", () => {
    const snapshot = new GameSession(null, createEmptyProfile(), "agent-judge-target").snapshot();
    const target = snapshot.monsters[0];
    expect(target).toBeDefined();
    const withObjective = (objectiveRoomId: string | null, hp = target!.hp) => ({
      ...snapshot,
      monsters: snapshot.monsters.map((monster) => (
        monster.id === target!.id ? { ...monster, hp } : monster
      )),
      navigationGuidance: { ...snapshot.navigationGuidance, objectiveRoomId },
    });

    expect(dungeonAgentGuidanceTargetKind(withObjective(null))).toBe("none");
    expect(dungeonAgentGuidanceTargetKind(withObjective("lesson:select"))).toBe("room");
    expect(dungeonAgentGuidanceTargetKind(withObjective(`area-boss:${target!.id}`))).toBe("live-monster");
    expect(dungeonAgentGuidanceTargetKind(withObjective(`area-boss:${target!.id}`, 0))).toBe("defeated-monster");
  });
