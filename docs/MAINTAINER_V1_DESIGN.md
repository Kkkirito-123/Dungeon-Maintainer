...
V1 非目标：多 Agent、Dashboard、Electron、面向用户的任意终端、任意浏览器脚本、自建模型循环、长期记忆、
自动发布和通用仓库适配。
...
检查和游戏语义工具；Agent 用 `finish(status=proposed)` 提交“病因 + 完整方案 + 验证方式 + 风险”后，
Shell 只询问一次是否执行。确认后当前 Agent 运行获得 Pi 原生 edit/write 和 patch；方案完成、
拒绝或 Agent 运行结束立即恢复只读集合。普通“继续”文本不是写入授权。
...
schema v3 保存任务路径、Hash、状态、模型档案、Thinking 等级、精确写入范围、有限检查/复现索引和结论。
读取时 v2 任务自动补齐 v3 字段并原子迁移；v1 任务明确拒绝恢复。状态机：
...
协议 v2 固定提供 `checkpoint/look/go/use/inputSql/query/judge/events`。桥只在
...
`bridge.ts` 保留旧导出作为兼容门面，但不再拥有投影、寻路或查询判定规则。
