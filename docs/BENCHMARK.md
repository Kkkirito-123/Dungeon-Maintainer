...
当前 Maintainer 已按 Pi 原生语义处理自然收尾：本轮没有下一次工具调用时直接收到
`agent_settled`，不会自动创建隐藏续跑或把自然结束升级成验证回合。代码修改仍保留在
隔离 worktree；需要检查时由用户显式执行 `/verify`，需要写回正式仓库时执行 `/apply`。

使用同一模型配置、同一游戏源码和 Pi `v0.84.2`，对 7 个真实修复 fixture 的单次矩阵结果如下。
Pi Original 数值来自已有基线归档；负数表示 Maintainer 更快。

| 指标 | Maintainer | Pi Original | 变化 |
|---|---:|---:|---:|
| 任务通过 | 7/7 | 7/7 | 持平 |
...
这是一组工程调试快照，不代表统计显著性，也不表示每个案例都更快；它用于验证当前
结束语义、工具回合和 Token 开销是否改善。完整归档默认位于 `benchmark-results/`，
该目录被 `.gitignore` 排除，不随代码提交。
