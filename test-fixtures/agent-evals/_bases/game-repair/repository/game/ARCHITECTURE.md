...
Schema v4 adds cross-layer `features`, a bounded runtime contract, and a stable
boundary signature to the stable layer/area/partition boundaries and eight
`floorScopes`; it remains routing data rather than a code or file index. Ordinary
file changes do not update the map.

Normal inspection routes by feature first and keeps the floor as optional context;
area and repository search are failure-open fallbacks. Only the complete current
schema v4 is accepted. Floor
