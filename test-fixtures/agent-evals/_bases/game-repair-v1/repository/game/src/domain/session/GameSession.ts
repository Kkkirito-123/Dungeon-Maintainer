 ...
 * 游戏运行时的唯一状态所有者。
 *
 * GameSession 负责移动、遭遇、战斗、伤害、篝火、死亡复活、剧情进度、
 * 奖励和存档转换；UI 与 Phaser 只能调用公开动作并读取 snapshot()，不能
 * 直接写入规则字段。Agent 只消费受限快照或复盘数据，不能成为状态来源。
 * 本类会产生快照通知，但不直接操作 DOM、浏览器存储或外部服务。
 */
import {
  JOIN_CHAIN,
  SORT_SABER,
...
import {
  ...
  type WorldActor,
} from "../exploration/monsterRoaming";
import {
  INITIAL_SAFE_STEPS,
...
import { rollLootItems } from "../inventory/lootDirector";
...
import { interactionFailure, travelFailure } from "./sessionInteraction";
...
export class GameSession {
  ...
  constructor(
    ...
    if (
      ...
      this.practiceDrawStates = savedRun.practiceDrawStates
        ? {
   ...
   * 把运行时状态转换为 v12 存档。
   *
   * 转换会复制可变集合和嵌套对象，保留旧存档字段以及 generatorVersion；
  ...
  private advanceGuidanceProgress(): boolean {
    ...
    const nextLevel: 0 | 1 | 2 | 3 = this.guidanceSteps >= NAVIGATION_RUNTIME_CONFIG.escortAt
      ? 3
      : this.guidanceSteps >= NAVIGATION_RUNTIME_CONFIG.routeHighlightAt
        ? 2
        : this.guidanceSteps >= NAVIGATION_RUNTIME_CONFIG.directionHintAt
          ? 1
          : 0;
  ...
  private usesQuestionBank(monster: Monster): boolean {
    const role = biomeEncounterFor(monster.id)?.role;
    return monster.encounterType === "ambush" && (
      role === "normal" || role === "mini-elite" || role === "area-boss"
    );
  }
  ...
  private preparePracticeBattle(monster: Monster): void {
    this.activePracticeMonsterId = null;
    this.activePracticeQuestionIds = [];
    if (!this.questionBank || !this.usesQuestionBank(monster)) return;
    ...
    const draw = this.questionBank.draw(
      this.floorNumber,
      this.runInstanceId,
      this.practiceDrawStates[tier],
      unlockedLessons,
      count,
      tier,
    );
    if (draw.questions.length === 0) return;
    this.practiceDrawStates[tier] = draw.state;
    this.activePracticeMonsterId = monster.id;
  ...
  private awardExperience(monster: Monster): ExperienceSettlement {
    return applyExperienceSettlement(monster, this.player, {
      experienceForRank,
      levelForXp,
      maxHpForLevel,
    });
  ...
  adminLoadFloor(floor: FloorNumber): InteractionResolution {
    ...
    if (!floorSeed) return this.interactionFailure("管理员预览层缺少固定地图标识。");
  ...
  adminApplyPreset(presetId: string): InteractionResolution {
    ...
    const focusLessonId = this.graph.nodes.find(
      (room) => room.id === focusLandmark.anchor.roomNodeId,
    )?.lessonId ?? null;
    ...
    this.groundItems = initialGroundItems(
      ...
      // Boss 门前预设仍需保留尚未完成课程的结算物，否则实机复现无法走到钥匙与传送阶段。
      (item) => isFloorOneChestItem(item)
        || !progressedRoomIds.has(item.sourceRoomId)
        || (
          focusLessonId !== null
          && !completedLessons.has(focusLessonId)
          && item.sourceRoomId === focusLandmark.anchor.roomNodeId
        ),
    );
    this.lootBundles = [];
    this.ensureOpenedHiddenAreaRewards();
